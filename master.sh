...
# 4. Install the Caloco Network Add-On:

kubectl apply -f

https://raw.githubusercontent.com/projectcalico/calico/v3.25.0/manifests/calico.yaml

# 5. Check the status of the control plane:

kubectl get nodes

# 6. Join the Worker Nodes to the Cluster:
# (In the control plane node, create the token)

kubeadm token create --print-join-command

# 7.Copy the full output from the 'kubeadm join" command  in # 6 above:
